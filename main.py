    ...
    if request.method == "POST":
        if form.validate_on_submit():
            if db.session.execute(db.select(User).where(User.email == request.form.get("email"))).scalar():
                flash("There is an existing account with this email. Log in instead!")
...
def login():
    ...
    if request.method == "POST":
        if login_form.validate_on_submit():
            account = db.session.execute(db.select(User).where(User.email == request.form.get("email"))).scalar()
            if account:
